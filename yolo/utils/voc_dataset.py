...
def plot_box(bboxes, img, id=None, color=None, line_thickness=None):
    ...
    for i, x in enumerate(bboxes):
        c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
        cv2.rectangle(img, c1, c2, color, thickness=tl)
        label = yolov3_voc.DATA["CLASSES"][int(x[4])]
        if label:
            tf = max(tl - 1, 1)  # font thickness
...
class VocDataset(Dataset):
    def __init__(self, anno_file_type, img_size=416):
        ...
        self.classes = yolov3_voc.DATA["CLASSES"]
    ...
    def __load_annotations(self, anno_type):
        ...
        anno_path = os.path.join(yolov3_voc.PROJECT_PATH, 'data',
    ...
    def __creat_label(self, bboxes):
        ...
        anchors = np.array(yolov3_voc.MODEL["ANCHORS"])
        strides = np.array(yolov3_voc.MODEL["STRIDES"])
        train_output_size = self.img_size / strides
        anchors_per_scale = yolov3_voc.MODEL["ANCHORS_PER_SCLAE"]
...
def parse_voc_annotation(data_path, file_type, anno_path,
    ...
    :return: 数据集大小
    """
    classes = yolov3_voc.DATA["CLASSES"]
...
if __name__ == "__main__":
    ...
    train_data_path_2007 = os.path.join(yolov3_voc.DATA_PATH,
                                        'VOCtrainval-2007', 'VOCdevkit',
                                        'VOC2007')
    train_data_path_2012 = os.path.join(yolov3_voc.DATA_PATH,
    ...
    test_data_path_2007 = os.path.join(yolov3_voc.DATA_PATH, 'VOCtest-2007',
                                       'VOCdevkit', 'VOC2007')
