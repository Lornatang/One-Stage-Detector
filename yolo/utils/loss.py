...
from . import GIOU_xywh_torch
from . import iou_xywh_torch
...
class YoloV3Loss(nn.Module):
    ...
    def __cal_loss_per_layer(self, p, p_d, label, bboxes, stride):
        ...
        # loss confidence
        iou = iou_xywh_torch(p_d_xywh.unsqueeze(4),
                                   bboxes.unsqueeze(1).unsqueeze(1).unsqueeze(
                                       1))
        iou_max = iou.max(-1, keepdim=True)[0]
        label_noobj_mask = (1.0 - label_obj_mask) * (
                    iou_max < self.iou_threshold).float()

        loss_conf = (label_obj_mask * FOCAL(input=p_conf,
                                            target=label_obj_mask) +
                     label_noobj_mask * FOCAL(input=p_conf,
                                              target=label_obj_mask)) * label_mix
