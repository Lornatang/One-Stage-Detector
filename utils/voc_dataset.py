...
def load_annotations(anno_type, cfg):
    assert anno_type in ['train',
                         'test'], "You must choice one of the 'train' or 'test' for anno_type parameter"
    anno_path = os.path.join(cfg.DATA_ROOT, anno_type + "_annotation.txt")
...
class VocDataset(Dataset):
    def __init__(self, anno_file_type, image_size=416, cfg=None):
        self.image_size = image_size  # For Multi-training
        self.cfg = cfg
        self.classes = self.cfg.CLASSES
        self.num_classes = len(self.classes)
        self.class_to_id = dict(zip(self.classes, range(self.num_classes)))
        self.annotations = load_annotations(anno_file_type, cfg)
    ...
    def __getitem__(self, item):
        ...
        img_org, bboxes_org = self.parse_annotation(self.annotations[item])
        img_org = img_org.transpose(2, 0, 1)  # HWC->CHW

        item_mix = random.randint(0, len(self.annotations) - 1)
        img_mix, bboxes_mix = self.parse_annotation(
            self.annotations[item_mix])
    ...
    def parse_annotation(self, annotation):
        ...
        img, bboxes = Resize((self.image_size, self.image_size), True)(
    ...
    def __creat_label(self, bboxes):
        ...
        strides = np.array(self.cfg.MODEL.STRIDES)
        train_output_size = self.image_size / strides
        anchors_per_scale = self.cfg.MODEL.PRIORS_ANCHOR
...
def parse_voc_annotation(data_path, file_type, anno_path,
                         use_difficult_bbox=False, cfg=None):
    """
...
if __name__ == "__main__":
    ...
    train_data_path_2007 = os.path.join("./data", 'VOCtrainval_06-Nov-2007',
                                        'VOCdevkit',
                                        'VOC2007')
    train_data_path_2012 = os.path.join("./data",
    ...
    test_data_path_2007 = os.path.join("./data", 'VOCtest_06-Nov-2007',
                                       'VOCdevkit', 'VOC2007')
    ...
    len_train += parse_voc_annotation(train_data_path_2012, "trainval",
                                      train_annotation_path,
                                      use_difficult_bbox=False)
